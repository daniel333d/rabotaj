  ...
  return (
    <div className="flex min-h-[70vh] items-center justify-center bg-surface px-6 py-16">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-white p-8 shadow-elevated">
        ...
        <h1 className="mt-4 font-heading text-xl font-extrabold text-ink">{t.auth.updatePasswordTitle}</h1>

        {state.success ? (
          <div className="mt-5 flex flex-col gap-4">
            <div className="flex items-start gap-2.5 rounded-xl border border-success/30 bg-green-50 p-3.5 text-sm text-ink" role="status">
              <CheckCircle2 size={16} className="mt-0.5 shrink-0 text-success" aria-hidden="true" />
              {t.auth.updatePasswordSuccess}
            </div>
            <Link
              href="/login"
              className="inline-flex items-center justify-center rounded-xl bg-brand px-5 py-3 text-sm font-semibold text-white transition-colors duration-200 hover:bg-blue-700"
            >
              {t.auth.loginLink}
            </Link>
          </div>
          ...
          <>
            ...
            <form action={formAction} className="mt-6 flex flex-col gap-4">
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-ink">{t.auth.password}</span>
                <input
                  name="password"
                  type="password"
                  required
              ...
              <label className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-ink">{t.auth.confirmPassword}</span>
                <input
              ...
              >
                {pending ? t.common.loading : t.auth.updatePasswordCta}
              </button>
            </form>
          </>
        )}
